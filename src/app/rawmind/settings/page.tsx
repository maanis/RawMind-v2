...
export default function RawMindSettingsPage() {
  const [ollamaUrl, setOllamaUrl] = useState(() => {
    ...
    return loadSettings().ollamaUrl;
  });
  const [model, setModel] = useState(() => {
    if (typeof window === "undefined") return OLLAMA_DEFAULT_MODEL;
    return loadSettings().model || OLLAMA_DEFAULT_MODEL;
  });
    ...
    <div className="min-h-screen px-6 py-10 md:py-16 max-w-2xl mx-auto space-y-10">
      ...
      <div className="card-surface rounded-2xl p-5 space-y-4">
        <div className="space-y-1.5">
          <label className="text-xs uppercase tracking-wide text-muted">Ollama server URL</label>
          <input
            value={ollamaUrl}
            onChange={(e) => {
            ...
            className="w-full bg-white/3 border border-border rounded-xl px-4 py-3 text-sm outline-none focus:border-(--accent)/40 transition-colors"
          />
          <p className="text-[11px] text-muted">
            Use <code className="text-foreground">http://localhost:11434</code> if this site
        ...
        <div className="space-y-1.5">
          <label className="text-xs uppercase tracking-wide text-muted">Model name</label>
          <input
            value={model}
        ...
        <div className="flex items-center gap-3 pt-1">
          <button
            ...
            className="flex items-center gap-2 text-xs px-4 py-2.5 rounded-full border border-border hover:border-white/20 disabled:opacity-30 transition-colors"
          >
      ...
      <div className="space-y-4">
        ...
        <ol className="space-y-4">
          ...
          <li className="card-surface rounded-xl p-4 space-y-1.5">
            ...
            <p className="text-xs text-muted">Open a terminal and run:</p>
            <pre className="bg-black/40 border border-border rounded-lg p-3 text-xs overflow-x-auto"><code>ollama pull dolphin-mixtral</code></pre>
            <p className="text-xs text-muted">
              Smaller machine? Try <code className="text-foreground">dolphin-mistral</code> or
          ...
          <li className="card-surface rounded-xl p-4 space-y-1.5">
            ...
            <p className="text-xs text-muted">
              ...
              <code className="text-foreground">http://localhost:11434</code>. You can check with:
            </p>
            <pre className="bg-black/40 border border-border rounded-lg p-3 text-xs overflow-x-auto"><code>curl http://localhost:11434/api/tags</code></pre>
          </li>
          ...
          <li className="card-surface rounded-xl p-4 space-y-1.5">
            ...
            <p className="text-xs text-muted">
              ...
              <code className="text-foreground">http://localhost:11434</code> above. If the
            ...
            <pre className="bg-black/40 border border-border rounded-lg p-3 text-xs overflow-x-auto"><code>ngrok http 11434</code></pre>
            <p className="text-xs text-muted">
              Then use the <code className="text-foreground">https://...ngrok-free.app</code>{" "}
