@echo off
REM RawMind -- one-click Windows setup (self-contained: this single file
REM contains the PowerShell script below its own marker line and extracts
REM + runs it, so there is nothing else to download).
setlocal
set "PS1FILE=%TEMP%\rawmind-setup.ps1"
for /f "tokens=1,* delims=:" %%A in ('findstr /n "^::PS1_MARKER::$" "%~f0"') do set "MARKERLINE=%%A"
set /a "PS1START=%MARKERLINE%+1"
more +%PS1START% "%~f0" > "%PS1FILE%"
